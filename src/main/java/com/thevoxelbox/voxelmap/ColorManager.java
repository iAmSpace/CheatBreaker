...
public class ColorManager
	...
	public final BufferedImage getBlockImage(int blockID, int metadata) {
		try {
			IIcon icon = ((Block) Block.blockRegistry.getObjectById(blockID)).getIcon(3, metadata);
	...
	private void loadSpecialColors() {
		...
		VoxelMapProtectedFieldsHelper.setLightOpacity(Block.getBlockFromName("net.minecraft:flowing_lava"), 1);
		VoxelMapProtectedFieldsHelper.setLightOpacity(Block.getBlockFromName("net.minecraft:lava"), 1);
	}
		...
		if (blockInts.length == 0) {
			...
			if (matcher.find()) {
				...
				for (int t = 0; t < 4096; t++) {
					Block block = (Block) Block.blockRegistry.getObjectById(t);
					if (block != null) {
	...
	private void processColorProperties() {
		...
		for (ResourceLocation resource : unusedPNGs) {
			...
			processColorProperty(resource, "net.minecraft:" + name);
		}
	}
