...
class GuiSlotWaypoints
	...
	protected void elementClicked(int par1, boolean par2, int par3, int par4) {
		...
		int width = 215;
		if ((this.mouseX >= leftEdge + width - 16 - padding) &&
	...
	protected void drawSlot(int par1, int par2, int par3, int par4, Tessellator par5Tessellator, int par6, int par7) {
		...
		byte padding = 3;
		if ((this.mouseX >= par2 - padding) && (this.mouseY >= par3) &&
		    (this.mouseX <= par2 + 215 + padding) && (this.mouseY <= par3 + this.slotHeight)) {
			String tooltip;
			if ((this.mouseX >= par2 + 215 - 16 - padding) && (this.mouseX <= par2 + 215 + padding)) {
